###################################################################################################
################################### NLTR Insertions within PC Genes 
#################
rm(list=ls(all=TRUE))
ls()
...
########## Define predictors (number of L1 and SINEs Insertions Within PC Genes)
...
corrNeo <- round(cor(mydata$NeoplasiaPrevalence, mydata$Transformed_L1_PC_Intersections),2)
corrNeo
corrNeo2 <- round(cor(mydata$NeoplasiaPrevalence, mydata$Transformed_L1_SINEs_PC_Intersections),2)
corrNeo2

corrMal <- round(cor(mydata$MalignancyPrevalence, mydata$Transformed_L1_PC_Intersections),2)
corrMal
corrMal2 <- round(cor(mydata$MalignancyPrevalence, mydata$Transformed_L1_SINEs_PC_Intersections),2)
corrMal2
...
########### Simple PGLS (Cancer ~ (Number of L1 OR All nLTR Insertions within PC Genes) + e)

### Neoplasia vs L1
NL1 <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_PC_Intersections, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1)
anova(NL1)
...
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_PC_Intersections , data=mydata)
...
model_lambda_1 <- pgls(NeoplasiaPrevalence ~ Transformed_L1_PC_Intersections, data = comparative_data, lambda = 1.00)
# Compare AIC values
...
NL1SINEs <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_SINEs_PC_Intersections, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1SINEs)
anova(NL1SINEs)
# Grab likelihood-based R2, Lambda, and p values for plotting
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_SINEs_PC_Intersections , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(NeoplasiaPrevalence ~ Transformed_L1_SINEs_PC_Intersections, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
#### Malignancy vs L1
ML1 <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_PC_Intersections, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1)
anova(ML1)
# Grab likelihood-based R2, Lambda, and p values for plotting
RelML1 <- R2_lik(ML1)
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(MalignancyPrevalence ~ Transformed_L1_PC_Intersections, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
ML1SINEs <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_SINEs_PC_Intersections, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1SINEs)
anova(ML1SINEs)
# Grab likelihood-based R2, Lambda, and p values for plotting
...
ML1SINEs_pvalue <- signif(ML1SINEs_pvalue[2,4], digits = 3)
ML1SINEs_pvalue
# Check for normality of residuals 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(MalignancyPrevalence ~ Transformed_L1_SINEs_PC_Intersections, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
########### (Cancer ~ (Number of L1 OR All nLTR Insertions within PC Genes) + Longevity + e) 
...
#cor(mydata$maximum_longevity_m, mydata$Transformed_L1_PC_Intersections)
#cor(mydata$maximum_longevity_m, mydata$Transformed_L1_SINEs_PC_Intersections)

#### Neoplasia vs L1 + Longevity
NL1Lon <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_PC_Intersections +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1Lon)
anova(NL1Lon)
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_PC_Intersections +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(NeoplasiaPrevalence ~ Transformed_L1_PC_Intersections + maximum_longevity_m, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
NL1SINEsLon <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_SINEs_PC_Intersections +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1SINEsLon)
anova(NL1SINEsLon)
# FDR test and extracting the adj. p-values from the model
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_SINEs_PC_Intersections +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(NeoplasiaPrevalence ~ Transformed_L1_SINEs_PC_Intersections + maximum_longevity_m, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
ML1Lon <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_PC_Intersections +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1Lon)
anova(ML1Lon)
# FDR test and extracting the adj. p-values from the model
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(MalignancyPrevalence ~ Transformed_L1_PC_Intersections +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(MalignancyPrevalence ~ Transformed_L1_PC_Intersections + maximum_longevity_m, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
ML1SINEsLon <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_SINEs_PC_Intersections +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1SINEsLon)
anova(ML1SINEsLon)
# FDR test and extracting the adj. p-values from the model
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(MalignancyPrevalence ~ Transformed_L1_SINEs_PC_Intersections +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(MalignancyPrevalence ~ Transformed_L1_SINEs_PC_Intersections + maximum_longevity_m, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
cat("AIC with Estimated Lambda:", round(AIC(ML1SINEs),2), "\n")
cat("AIC with Lambda = 1:", round(AIC(model_lambda_1),2), "\n")

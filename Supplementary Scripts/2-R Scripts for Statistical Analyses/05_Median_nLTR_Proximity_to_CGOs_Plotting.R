################################################################################
####################### Plotting Median Proximity of nLTRs to CGOs
########

################################################### 
###  Neoplasia vs Median L1 Proximity to CGOs
################################################### 
...
# Final Plot
plot <- ggplot(mydata, aes(x = Transformed_L1_Medians_to_HCGHs, y = NeoplasiaPrevalence)) +
  ...
  # Axis labels
  xlab("Medians Proximity of Active L1s to CGOs")+
...
# Save PNG
ggsave("17_L1_Medians_to_HCGHs_vs_Neoplasia_N.png", plot, width = 10, height = 8, dpi = 800)
# Command to save a ggplot as an R object:
saveRDS(plot, "17_L1_Medians_to_HCGHs_vs_Neoplasia_N.rds")

################################################### 
###  Neoplasia vs Median L1-SINEs Proximity to CGOs
################################################### 
...
# Make labels to be above or under the images according to their coordinate toward fitted line
fitted_values <- predict(Current_model)
mydata$nudge_y <- ifelse(Current_Response > fitted_values, 0.02, -0.02)

# Create a new column to classify orders as "Other" or keep specific orders
mydata$color_group <- ifelse(mydata$Order %in% c("Rodentia", "Artiodactyla", "Carnivora", "Chiroptera", "Primates"),
                             mydata$Order,
                             "Other Orders")
...
# Final Plot
plot <- ggplot(mydata, aes(x = Transformed_L1_SINEs_Medians_to_HCGHs, y = NeoplasiaPrevalence)) +
  
  # Colored dots instead of silhouettes
  geom_point(aes(color = color_group), size = 3, alpha = 0.9) +
  
  # Species labels (memorized format)
  geom_text_repel(
    data = mydata,
    aes(label = Common_Name),
    size = 2.5,
  ...
  # Axis labels
  xlab("Medians Proximity of Active L1 and SINEs to CGOs")+
  ylab("Neoplasia Prevalence")+
  
  # Manual colors and model stats legend
  scale_color_manual(
    name = paste0(
      "<b style='font-size:11pt;'>Model Stats</b><br>",
      nrow(mydata), " species<br>",
  ...
  # Axis limits
  scale_x_continuous(limits = c(-0.08, 0.8), expand = c(0, 0)) +
  scale_y_continuous(limits = c(0.1, 0.97), expand = c(0, 0)) +
...
# Save PNG
ggsave("18_L1_SINEs_Medians_to_HCGHs_vs_Neoplasia_N.png", plot, width = 10, height = 8, dpi = 800)
# Command to save a ggplot as an R object:
saveRDS(plot, "18_L1_SINEs_Medians_to_HCGHs_vs_Neoplasia_N.rds")


################################################### 
###  Malignancy vs Median L1 Proximity to CGOs
################################################### 
...
# Make labels to be above or under the fitted line
fitted_values <- predict(Current_model)
mydata$nudge_y <- ifelse(Current_Response > fitted_values, 0.02, -0.02)
...
# Final Plot
plot <- ggplot(mydata, aes(x = Transformed_L1_Medians_to_HCGHs, y = MalignancyPrevalence)) +
  
  # Colored dots
  geom_point(aes(color = color_group), size = 3, alpha = 0.9) +
  
  # Species labels (all species, size = 3)
  geom_text_repel(
    data = mydata,
    aes(label = Common_Name),
    size = 2.5,
...
# Save PNG
ggsave("19_L1_Medians_to_HCGHs_vs_Malignancy.png", plot, width = 10, height = 8, dpi = 800)
# Save as R object
saveRDS(plot, "19_L1_Medians_to_HCGHs_vs_Malignancy.rds")


###################################################### 
###  Malignancy vs Median L1-SINEs Proximity to CGOs
###################################################### 
...
# Generate fitted values to calculate label nudging (no longer used)
fitted_values <- predict(Current_model)
mydata$nudge_y <- ifelse(Current_Response > fitted_values, 0.02, -0.02)
...
# Final Plot
plot <- ggplot(mydata, aes(x = Transformed_L1_SINEs_Medians_to_HCGHs, y = MalignancyPrevalence)) +
  
  # Colored dots
  geom_point(aes(color = color_group), size = 3, alpha = 0.9) +
  
  # Species labels (all species, size = 3, no nudge_y)
  geom_text_repel(
    data = mydata,
    aes(label = Common_Name),
    size = 2.5,
...
# Save PNG
ggsave("20_L1_SINEs_Medians_to_HCGHs_vs_Malignancy.png", plot, width = 10, height = 8, dpi = 800)
# Save as R object
saveRDS(plot, "20_L1_SINEs_Medians_to_HCGHs_vs_Malignancy.rds")

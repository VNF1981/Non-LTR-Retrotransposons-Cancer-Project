...
suppressPackageStartupMessages({
  ...
  library(outliers) # To identify outliers (Grub's test)
  ...
  library(rr2) # For reading tree / Likelihood-based R2
...
########## Define predictor variables (L1 counts and combined L1 + SINE counts)
...
corrNeo <- round(cor(mydata$NeoplasiaPrevalence, mydata$Transformed_L1_Counts),2)
corrNeo
corrNeo2 <- round(cor(mydata$NeoplasiaPrevalence, mydata$Transformed_L1_SINEs_Counts),2)
corrNeo2

corrMal <- round(cor(mydata$MalignancyPrevalence, mydata$Transformed_L1_Counts),2)
corrMal
corrMal2 <- round(cor(mydata$MalignancyPrevalence, mydata$Transformed_L1_SINEs_Counts),2)
corrMal2
...
########### Simple PGLS (Cancer ~ (Number of L1 OR All nLTRs) + e)

### Neoplasia vs L1
NL1 <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_Counts, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1)
anova(NL1)
...
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_Counts , data=mydata)
...
model_lambda_1 <- pgls(NeoplasiaPrevalence ~ Transformed_L1_Counts, data = comparative_data, lambda = 1.00)
# Compare AIC values
...
NL1SINEs <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_SINEs_Counts, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1SINEs)
anova(NL1SINEs)
# Grab likelihood-based R2, Lambda, and p values for plotting
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_SINEs_Counts , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#### Malignancy vs L1
ML1 <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_Counts, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1)
anova(ML1)
# Grab likelihood-based R2, Lambda, and p values for plotting
RelML1 <- R2_lik(ML1)
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(MalignancyPrevalence ~ Transformed_L1_Counts , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(MalignancyPrevalence ~ Transformed_L1_Counts, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
ML1SINEs <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_SINEs_Counts, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1SINEs)
anova(ML1SINEs)
# Grab likelihood-based R2, Lambda, and p values for plotting
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(MalignancyPrevalence ~ Transformed_L1_SINEs_Counts , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(MalignancyPrevalence ~ Transformed_L1_SINEs_Counts, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
########### (Neoplasia or Malignancy ~ (Number of L1 OR All nLTRs) + Longevity + e) 
...
# We did not remove two influential points in our analyses. 
...
NL1Lon <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_Counts +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1Lon)
anova(NL1Lon)
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_Counts +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(NeoplasiaPrevalence ~ Transformed_L1_Counts + maximum_longevity_m, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
NL1SINEsLon <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_SINEs_Counts +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1SINEsLon)
anova(NL1SINEsLon)
# FDR test and extracting the adj. p-values from the model
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_SINEs_Counts +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(NeoplasiaPrevalence ~ Transformed_L1_SINEs_Counts + maximum_longevity_m, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
ML1Lon <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_Counts +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1Lon)
anova(ML1Lon)
# FDR test and extracting the adj. p-values from the model
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(MalignancyPrevalence ~ Transformed_L1_Counts +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(MalignancyPrevalence ~ Transformed_L1_Counts + maximum_longevity_m, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
...
ML1SINEsLon <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_SINEs_Counts +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1SINEsLon)
anova(ML1SINEsLon)
# FDR test and extracting the adj. p-values from the model
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(MalignancyPrevalence ~ Transformed_L1_SINEs_Counts +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#                                      vcv = TRUE, na.omit = FALSE)
# Lambda fixed at 1 (full phylogenetic signal)
model_lambda_1 <- pgls(MalignancyPrevalence ~ Transformed_L1_SINEs_Counts + maximum_longevity_m, data = comparative_data, lambda = 1.00)
# Compare AIC values
cat("AIC with Lambda = 0:", round(AIC(model),2), "\n")
cat("AIC with Estimated Lambda:", round(AIC(ML1SINEs),2), "\n")
cat("AIC with Lambda = 1:", round(AIC(model_lambda_1),2), "\n")

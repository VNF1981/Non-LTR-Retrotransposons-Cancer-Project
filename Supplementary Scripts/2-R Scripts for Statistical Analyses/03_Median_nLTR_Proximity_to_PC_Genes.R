###################################################################################################
############################## nLTR Proximity to PC Genes Analyses (Median)
#################
rm(list=ls(all=TRUE))
ls()
...
########## Define predictors (Median of L1 and SINEs to PC Genes)
mydata$Transformed_L1_Medians_to_PC_Genes <- mydata$L1_Medians_to_PC_Genes
mydata$Transformed_L1_SINEs_Medians_to_PC_Genes <- mydata$L1_SINEs_Medians_to_PC_Genes
...
hist(mydata$Transformed_L1_Medians_to_PC_Genes)
#mydata$Transformed_L1_Medians_to_PC_Genes <- transformTukey(mydata$Transformed_L1_Medians_to_PC_Genes)
# scale data to the 0,1 range
mydata$Transformed_L1_Medians_to_PC_Genes <- (mydata$Transformed_L1_Medians_to_PC_Genes - min(mydata$Transformed_L1_Medians_to_PC_Genes)) / (max(mydata$Transformed_L1_Medians_to_PC_Genes) - min(mydata$Transformed_L1_Medians_to_PC_Genes))
hist(mydata$Transformed_L1_Medians_to_PC_Genes)

hist(mydata$Transformed_L1_SINEs_Medians_to_PC_Genes)
#mydata$Transformed_L1_SINEs_Medians_to_PC_Genes <- transformTukey(mydata$Transformed_L1_SINEs_Medians_to_PC_Genes)
# scale data to the 0,1 range
mydata$Transformed_L1_SINEs_Medians_to_PC_Genes <- (mydata$Transformed_L1_SINEs_Medians_to_PC_Genes - min(mydata$Transformed_L1_SINEs_Medians_to_PC_Genes)) / (max(mydata$Transformed_L1_SINEs_Medians_to_PC_Genes) - min(mydata$Transformed_L1_SINEs_Medians_to_PC_Genes))
hist(mydata$Transformed_L1_SINEs_Medians_to_PC_Genes)


########### Check for the potential outliers in nLTR measurements
grubbs_result <- grubbs.test(mydata$Transformed_L1_Medians_to_PC_Genes)
print(grubbs_result)
grubbs_result <- grubbs.test(mydata$Transformed_L1_SINEs_Medians_to_PC_Genes)
print(grubbs_result)

########### To remove one outlier in L1 data
mydata <- mydata[mydata$Transformed_L1_Medians_to_PC_Genes != (max(mydata$Transformed_L1_Medians_to_PC_Genes)), ]
mydata <- mydata[mydata$Transformed_L1_Medians_to_PC_Genes != (max(mydata$Transformed_L1_Medians_to_PC_Genes)), ]
nrow(mydata)

########## Pearson Correlations Between Neoplasia/Malignancy and nLTR measurements
corrNeo <- round(cor(mydata$NeoplasiaPrevalence, mydata$Transformed_L1_Medians_to_PC_Genes),2)
corrNeo
corrNeo2 <- round(cor(mydata$NeoplasiaPrevalence, mydata$Transformed_L1_SINEs_Medians_to_PC_Genes),2)
corrNeo2

corrMal <- round(cor(mydata$MalignancyPrevalence, mydata$Transformed_L1_Medians_to_PC_Genes),2)
corrMal
corrMal2 <- round(cor(mydata$MalignancyPrevalence, mydata$Transformed_L1_SINEs_Medians_to_PC_Genes),2)
corrMal2
...
### Neoplasia vs L1
NL1 <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_Medians_to_PC_Genes, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1)
anova(NL1)
...
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_Medians_to_PC_Genes , data=mydata)
...
NL1SINEs <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_SINEs_Medians_to_PC_Genes, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1SINEs)
anova(NL1SINEs)
# Grab likelihood-based R2, Lambda, and p values for plotting
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_SINEs_Medians_to_PC_Genes , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
#### Malignancy vs L1
ML1 <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_Medians_to_PC_Genes, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1)
anova(ML1)
# Grab likelihood-based R2, Lambda, and p values for plotting
RelML1 <- R2_lik(ML1)
...
ML1SINEs <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_SINEs_Medians_to_PC_Genes, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1SINEs)
anova(ML1SINEs)
# Grab likelihood-based R2, Lambda, and p values for plotting
...
#cor(mydata$maximum_longevity_m, mydata$Transformed_L1_Medians_to_PC_Genes)
#cor(mydata$maximum_longevity_m, mydata$Transformed_L1_SINEs_Medians_to_PC_Genes)

#### Neoplasia vs L1 + Longevity
NL1Lon <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_Medians_to_PC_Genes +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1Lon)
anova(NL1Lon)
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_Medians_to_PC_Genes +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
NL1SINEsLon <- pglsSEyPagel(NeoplasiaPrevalence ~ Transformed_L1_SINEs_Medians_to_PC_Genes +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(NL1SINEsLon)
anova(NL1SINEsLon)
# FDR test and extracting the adj. p-values from the model
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(NeoplasiaPrevalence ~ Transformed_L1_SINEs_Medians_to_PC_Genes +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
ML1Lon <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_Medians_to_PC_Genes +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1Lon)
anova(ML1Lon)
# FDR test and extracting the adj. p-values from the model
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(MalignancyPrevalence ~ Transformed_L1_Medians_to_PC_Genes +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
...
ML1SINEsLon <- pglsSEyPagel(MalignancyPrevalence ~ Transformed_L1_SINEs_Medians_to_PC_Genes +  maximum_longevity_m, data = mydata, tree=tre, se=SE, method = "ML")
summary(ML1SINEsLon)
anova(ML1SINEsLon)
# FDR test and extracting the adj. p-values from the model
...
# Autocorrelation occurs when the residuals of a regression model are correlated
# with each other, indicating that there is some pattern or dependence among the errors.
model <- lm(MalignancyPrevalence ~ Transformed_L1_SINEs_Medians_to_PC_Genes +  maximum_longevity_m , data=mydata)
durbinWatsonTest(model) #Durbin-Watson test using car package
# Check for heteroscedasticity (Breusch-Pagan test)
# Checks if the variance of the errors is the same for every value of x (homoscedasticity). 
# Look for a random scatter of points around zero, with no discernible pattern or trend. 
